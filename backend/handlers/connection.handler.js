export class ConnectionHandler {
  ...
  handleSkipUser(socket, data) {
    ...
    console.log(`User B (${socket.id}) skipping User A (${session.userA}) in room ${roomId}`);
    ...
    setTimeout(() => {
      ...
      console.log(`User B re-queued, match attempt result: ${matched ? 'success' : 'no match'}`);
    }, 100);
  }
  ...
  handleEndSession(socket, data) {
    ...
    console.log(`User B (${socket.id}) ending session with User A (${session.userA}) in room ${roomId}`);
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        const matched = this.matchingService.matchUsers();
        console.log(`User B re-queued after session end, match attempt result: ${matched ? 'success' : 'no match'}`);
      }, 200);
    }, 100);
  }
  ...
  handleDisconnect(socket) {
    ...
    if (sessionInfo) {
      ...
      const otherUserId = session.userA === socket.id ? session.userB : session.userA;
      ...
      if (session.userA === socket.id && otherUserSocket) {
        setTimeout(() => {
          const position = this.queueService.addUserB(otherUserId, otherUserSocket);
